...
namespace Cyril_and_Methodius.Model
{
    ...
    {"zip", "application/zip"}
            ...
            public static bool GetMIMEType(string extension)
            {
                if (extension.Length > 0 && MIMETypesDictionary.ContainsValue(extension))
                {
                    foreach (var item in SupportedMIME)
                    {
                        if (item == extension)
                        {
                        return true;
        ...
        private static string[] SupportedMIME =
        {
            "text/plain"//add supported types here
        };
    }
}
